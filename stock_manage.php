        ...
        <div class="content">
            ...
            <div class="main-body">
                ...
                if(isset($_POST['submit'])){
                    ...
                    if(mysqli_query($link,"INSERT INTO ".$prefix."stock_manage SET `sc_id`='".$_POST['item_id']."',`stock`='".$_POST['stock']."',`type`='infused',`ref_id`='0'")){
                       ...
                       echo '<p class="alert alert-success"><i class="fa fa-check fa-fw"></i>Added successfully.</p>';
                        ...
                        echo '<p class="alert alert-danger"><i class="fa fa-warning fa-fw"></i> Sorry! There is an error while uploading image!</p>'.mysqli_error(); 
                    }
                ...
                if(isset($_POST['usubmit'])){
                    ...
                    //check and insert banner
                    $query='';
                    foreach($_POST as $key=>$value) if($key != 'usubmit')if($key != 'qa')$query .="`$key`='$value',";
                    $query = substr($query, 0, -1);//omit last comma
                    
                    if(mysqli_query($link,"INSERT INTO ".$prefix."stock_manage SET `sc_id`='".$_POST['item_id']."',`stock`='".$_POST['stock']."',`type`='deducted',`ref_id`='0'")){
                        

                        $stock = intval(anything('subcategory','stock',$_POST['item_id'])) - intval($_POST['stock']);
                        mysqli_query($link,"UPDATE ".$prefix."subcategory SET `stock`='".$stock."' WHERE `id`=".$_POST['item_id']);
                       echo '<p class="alert alert-success"><i class="fa fa-check fa-fw"></i>Deducted successfully.</p>';
                    }
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <table id="Projects_list" class="table table-hover table-bordered table-condensed" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                <?php
                                $select=mysqli_query($link,"SELECT * FROM ".$prefix."subcategory WHERE `del`='0'");
                                ...
                                <tr class="<?=$row->stock < 5 ?'bg-danger':''?>">
                                    ...
                                    <td>  
                                        ...
                                        <button class="btn btn-default btn-sm" title="Deduct stock" onClick="getItemInvoice1(<?=$row->id?>)"><i class="fa fa-minus fa-fw"></i></button> 
                                    </td>
                                </tr>
                                <?php } ?>
                            </tbody>
                  ...
                  <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body clearfix">                           
                         ...
                         <div class="modal fade deduct" tabindex="-1" role="dialog">
                  <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                            <h4 class="mod-title">Deduct Stock</h4>
                        </div>
                        <div class="modal-body clearfix">                           
                                <form action="" class="form-horizontal" method="post" enctype="multipart/form-data">
                                    <input type="hidden" id="item_id1" name="item_id" required>
