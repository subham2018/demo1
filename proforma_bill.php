...
<?php include_once 'include/navbar.php';?>	
        <div class="content">
            ...
            <div class="top">
            	<p class="page-title"><i class="fa fa-inr"></i>View Proforma Invoice</p>
                <ol class="breadcrumb">
            ...
            <div class="main-body">
                ...
                if(isset($_POST['submit'])){            
                    ...
                    if(mysqli_query($link,"INSERT INTO ".$prefix."proforma_bill SET `pname`='".$pname."',`pemail`='".$pemail."',`pphone`='".$pphone."', ".$query)){
                       $id=mysqli_insert_id($link);
                        foreach ($_POST['item'] as $key => $value) {
                            ...
                            mysqli_query($link,"INSERT INTO ".$prefix."proforma_invoice SET ".$query);
                            if(is_numeric($value)){
                              //  mysql_query("INSERT INTO ".$prefix."stock_manage SET `sc_id`='".$value."',`stock`='".$_POST['qty'][$key]."',`type`='cleared',`ref_id`='".mysql_insert_id()."'");

                                $stock = floatval(anything('subcategory','stock',$value)) - floatval($_POST['qty'][$key]);
                              //  mysql_query("UPDATE ".$prefix."subcategory SET `stock`='".$stock."' WHERE `id`=".$value);
                            }
                ...
                if(isset($_POST['usubmit'])){            
                    ...
                    if(mysqli_query($link,"UPDATE ".$prefix."proforma_bill SET `pname`='".$pname."',`pemail`='".$pemail."',`pphone`='".$pphone."', ".$query." WHERE `id`='".$_POST['inv_id']."'")){
                       $id=$_POST['inv_id'];
                        foreach ($_POST['item'] as $key => $value) {
                            $iname=anything('subcategory','name',$value);
                            $ides=anything('subcategory','dsc',$value);
...
//mysql_query("INSERT INTO ".$prefix."stock_manage SET `sc_id`='".$value."',`stock`='".$_POST['qty'][$key]."',`type`='cleared',`inv_id`='".$id."',`ref_id`='".mysql_insert_id()."'");
                                ...
                                $prev_item = row('proforma_invoice',$_POST['iid'][$key]);
                                if(is_numeric($value))
                                ...
                                mysqli_query($link,"UPDATE ".$prefix."proforma_invoice SET ".$query." WHERE `id`='".$_POST['iid'][$key]."'");

                                if(is_numeric($value)){
                                   // mysql_query("UPDATE ".$prefix."stock_manage SET `sc_id`='".$value."',`stock`='".$_POST['qty'][$key]."',`type`='cleared', `inv_id`='".$id."' WHERE `ref_id`='".$_POST['iid'][$key]."'");
                                    ...
                                    if($prev_item->item_id == $value){
                                       ...
                                       // mysql_query("UPDATE ".$prefix."subcategory SET `stock`='".$stock."' WHERE `id`=".$prev_item->item_id);
                        ...
                        $s = mysqli_query($link,"SELECT * FROM `".$prefix."proforma_invoice` where `inv_id`='".$_POST['inv_id']."'");
                        while ($r=mysqli_fetch_object($s)) {
                           if(!in_array($r->id, $_POST['iid'])){
                              ...
                              //  mysql_query("UPDATE ".$prefix."subcategory SET `stock`='".$stock."' WHERE `id`=".$r->item_id);
                                mysqli_query($link,"DELETE FROM `".$prefix."proforma_invoice` where `id`='".$r->id."'");
                           }
                       }   
                ...
                if(isset($_GET['del'])){
                    ...
                    if(mysqli_query($link,"UPDATE ".$prefix."proforma_bill SET `cancel`='1' WHERE id=".variable_check($_GET['del']))){ //delete info
                mysqli_query($link,"DELETE  FROM ".$prefix."proforma_invoice WHERE inv_id=".variable_check($_GET['del']));
                 //mysql_query("DELETE  FROM ".$prefix."stock_manage WHERE inv_id=".variable_check($_GET['del']));
                ...
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <div class="panel-title"><i class="fa fa-list fa-fw"></i> Proforma Invoice List <a href="provoice.php" class="btn btn-sm btn-default pull-right">Create Invoice</a></div>
                    </div>
                    <div class="panel-body">
                        <table id="Projects_list" class="table table-hover table-bordered table-condensed" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                <?php
                                $select=mysqli_query($link,"SELECT * FROM ".$prefix."proforma_bill");
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a class="btn btn-default btn-sm" href="provoice_edit.php?id=<?=$row->id?>"><i class="fa fa-eye fa-fw"></i>/<i class="fa fa-pencil fa-fw"></i></a>        
                                        <div class="btn-group">
                                          <button type="button" class="btn btn-success btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                            <i class="fa fa-fw fa-print"></i> <span class="caret"></span>
                                          </button>
                                          <ul class="dropdown-menu dropdown-menu-right">
                                            <li><a href="proforma_print.php?id=<?=$row->id?>&t=p" target="_blank">PDF / PRINT</a></li>
